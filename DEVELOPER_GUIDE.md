...
**Seam/system tests against real Postgres** (`__tests__/{seam,system}/*.test.ts`, run via `npm run test:integration`) use real, disposable scratch databases (`test-support/db/scratchDatabase.ts`) rather than fakes — see `TESTING_PLAN.md` §1.2 for why. Two things worth knowing before writing another one:
