...
**The comparison worth studying:** `users.email` is also a string identity, against a plain `UNIQUE` column that would happily store `A@b.com` and `a@b.com` as two accounts. Canonicalization lives in `controlDb.normalizeEmail`, applied inside `createUser` and `findUserByEmail` — so it is a property of the storage boundary, not of remembering to call it.

It did not start that way, and the way it failed is the useful part. Each caller normalized for itself, and only two of the four still did: `registerUser` and `authenticate` lower-cased, while `changePassword` and `deleteAccount` passed their email straight through. Those two were *correct anyway*, because their address reaches them from the session already canonical — so nothing failed, no test complained, and the rule quietly stopped being true. That is the characteristic failure of per-caller normalization: it doesn't break where it's wrong, it breaks later, when someone adds a fifth caller that passes user-typed text and gets "no such account" for an account that plainly exists.

No user was ever affected, and it is worth being precise about why: `createUser` would have inserted a mixed-case address verbatim, leaving `UNIQUE` blind to the collision and creating two accounts for one person — but its only caller lower-cased first, so nothing reached it. The bug was latent in the contract, not live in the app. That is the normal state of this hazard right up until it isn't. `__tests__/seam/controlDb.seam.test.ts` now pins both directions — lookup by any casing, and refusal of a second account differing only in case — against real Postgres.

Hence the rule: **when identity is a string, give it one named normalization function and apply it at the storage boundary** — not at the call sites you happen to be looking at. `cleanDiscogsArtistName` is the app's other example; it is the single place a Discogs artist name is canonicalized, which is why teaching it "Various" → "Various Artists" was a one-line edit.
...
- **The control plane** (the `control` schema, one, shared) holds only `users` and `sessions`. It's accessed via a hand-written `pg.Pool` in `lib/controlDb.ts`, not Prisma — two tables didn't justify a second Prisma schema/client. Its schema and tables are created idempotently (`CREATE SCHEMA` / `CREATE TABLE IF NOT EXISTS`) the first time the pool is used, so there's nothing to run by hand. The pool's `search_path` names the schema, so the queries themselves are unqualified.
  ...
  **Gotcha: bootstrap DDL must take table locks in the same order the hot path does.** The whole batch is one implicit transaction, so every lock it takes is held to the end — and it runs at module load, meaning it races ordinary requests on every process start. When an `ALTER TABLE sessions` was added alongside the existing `ALTER TABLE users`, the batch reached `users` first and `sessions` second, while `findSessionByTokenHash`'s `FROM sessions JOIN users` locks them the other way. That inversion is a textbook deadlock, and it reached a user as `deadlock detected` on a page load (2026-08-07). The batch now `LOCK`s both tables up front in the query's order — `sessions`, then `users` — which removes the cycle: a concurrent reader either finishes first or blocks holding nothing. **If you add DDL for another control-plane table, check the order against every query that joins it.** `__tests__/seam/controlDbBootstrap.seam.test.ts` holds a reader open across a real concurrent bootstrap and fails if the ordering regresses.
...
**Admin dashboard (`/admin`):** A separate, single account (username `admin`, hardcoded — there's only ever one admin, so a second environment variable for it wouldn't buy anything) for viewing all registered accounts. Its password is `ADMIN_PASSWORD` (`lib/adminCredentials.ts`), read from the environment and defaulting to blank when unset — meant to be replaced with a real value before this app is exposed beyond localhost; `/admin/page.tsx` shows a warning banner for as long as it stays blank. There's no database row or recovery flow behind this password — see the README's "Updating the Admin Password" section for the actual workflow (edit `.env`, restart outside of `next dev`) and the caution about saving a new value somewhere durable before setting it. It's intentionally isolated from the per-user auth system:
