 ...
 * Any test that reloads controlDb.ts against a different database — directly, or via
 * a caller like registerUser.ts that imports it — must reset this same cache, or the
 * next load reuses a pool still bound to the previous database, and the next close
